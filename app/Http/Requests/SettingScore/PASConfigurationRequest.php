...
class PASConfigurationRequest extends FormRequest
{
    /**
    ...
    public function rules()
    {
        return [
            ...
            'score_uas.*' => 'required|integer|min:0|max:100',
        ];
    }
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            $scoreUas = $this->input('score_uas');

            foreach ($averageDailyRates as $key => $rate) {
                $total = intval($rate) + intval($scoreUts[$key]) + intval($scoreUas[$key]);
